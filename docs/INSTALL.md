...
Before running the example, first add the entire `examples/` directory (containing the sample script, helper functions, and data) to the MATLAB search path. This ensures that MATLAB can
access all required files.

Next, open `example_reconstruction.m`, place a breakpoint on the `loadlibrary(dllPath, headerPath);`
line, and start the script. When execution stops at the breakpoint, verify that the variables
